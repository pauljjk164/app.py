        ...
        if invites >= BONUS_QUESTIONS_THRESHOLD:
            ...
            if bonus_questions != current_bonus:
                ...
                if bonus_questions > current_bonus:
                    ...
                    logger.info(f"Awarded ₱{BONUS_CASH_REWARD} bonus to user {user_id} for reaching {BONUS_QUESTIONS_THRESHOLD} invites")
...
def send_promotional_message():
    """Send promotional message to all verified users every 15 minutes"""
    try:
        ...
        for user_id, invite_code in users:
            try:
                ...
                invite_link = f"https://t.me/{updater.bot.username}?start={invite_code}"
                
                # Send promotional message
                updater.bot.send_message(
                    chat_id=user_id,
                    text=f"💸 *SECRET OFFER: EXTRA CASH!* 💸\n\n"
                         "Want some extra cash? Don't tell anyone!\n\n"
                         "📢 **Special Invite Rewards:**\n"
                         "• Invite 20 people = Get extra ₱5000\n"
                         "• Invite 30 people to our partner = Get ₱10000\n\n"
                         "📝 **How to get it?**\n"
                         "1. Invite people using your invite link below\n"
                         "2. We'll track your invites automatically\n"
                         "3. When you reach the target, we'll add the bonus to your balance\n"
                         "4. You can withdraw it immediately!\n\n"
                         f"🔗 Your invite link: {invite_link}\n"
                         f"🔗 Partner link: {ARENA_LIVE_LINK}\n\n"
                         "⚠️ *This offer is exclusive and time-limited!*",
                    parse_mode="Markdown",
...
def main() -> None:
    ...
    scheduler.start()
    logger.info("Promotional message scheduler started - messages will be sent every 15 minutes")
    ...
    dispatcher.add_handler(MessageHandler(Filters.regex('^🧊 FREEZE CASH$'), show_freeze_cash))
