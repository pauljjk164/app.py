...
BONUS_QUESTIONS_PER_INVITE = 5  # Bonus questions awarded per threshold
BONUS_QUESTIONS_THRESHOLD = 5  # Invites needed for bonus questions
BONUS_CASH_REWARD = 555  # Cash reward when reaching invite threshold
...
# Conversation states
(VERIFICATION, CHANNEL_JOIN, CAPTCHA_GAME, WITHDRAW_AMOUNT, WITHDRAW_INFO,
 QUIZ_GAME) = range(6)
...
if c.fetchone()[0] == 0:
    sample_questions = [
        ("What is the capital of the Philippines?", "Manila", "Cebu", "Davao",
         "Quezon City", 1),
        ("Which color is not in the Philippine flag?", "Red", "Blue", "Yellow",
         "Green", 4),
        ("What is the national bird of the Philippines?", "Eagle", "Maya",
         "Parrot", "Crow", 1),
        ("How many islands are there in the Philippines?", "7,107", "5,000",
         "10,000", "3,500", 1),
        ("What is the national fruit of the Philippines?", "Mango", "Banana",
         "Pineapple", "Papaya", 1),
        ("Which sea surrounds the Philippines?", "South China Sea",
         "Caribbean Sea", "Mediterranean Sea", "Baltic Sea", 1),
        ("What is the currency of the Philippines?", "Peso", "Dollar", "Euro",
         "Yen", 1),
        ("Which festival is celebrated in Cebu?", "Sinulog", "Ati-Atihan",
         "Dinagyang", "Pahiyas", 1),
        ("What is the tallest mountain in the Philippines?", "Mt. Apo",
         "Mt. Pulag", "Mt. Mayon", "Mt. Pinatubo", 1),
        ("Which Philippine president served the longest?", "Ferdinand Marcos",
         "Gloria Arroyo", "Rodrigo Duterte", "Benigno Aquino", 1),
        ("What is the national flower of the Philippines?", "Sampaguita",
         "Rose", "Orchid", "Tulip", 1),
        ("Which Philippine hero is known as the 'Great Plebeian'?",
         "Andres Bonifacio", "Jose Rizal", "Apolinario Mabini",
         "Emilio Aguinaldo", 1),
        ("What is the traditional Filipino Christmas greeting?",
         "Maligayang Pasko", "Salamat", "Kumusta", "Paalam", 1),
        ("Which province is known as the 'Rice Granary of the Philippines'?",
         "Nueva Ecija", "Pampanga", "Bulacan", "Tarlac", 1),
        ("What is the traditional Filipino martial art?", "Arnis", "Karate",
         "Taekwondo", "Kung Fu", 1),
        ("Which Philippine city is known as the 'City of Smiles'?", "Bacolod",
         "Cebu", "Davao", "Manila", 1),
        ("What is the most popular Filipino dessert?", "Halo-halo",
         "Leche Flan", "Buko Pie", "Mais Con Yelo", 1),
        ("Which Philippine island is known for its chocolate hills?", "Bohol",
         "Palawan", "Boracay", "Siargao", 1),
        ("What is the national tree of the Philippines?", "Narra", "Acacia",
         "Mahogany", "Mango Tree", 1),
        ("Which Philippine volcano is known for its perfect cone shape?",
         "Mayon", "Taal", "Pinatubo", "Kanlaon", 1),
        ("What is the traditional Filipino boat?", "Bangka", "Yacht",
         "Speedboat", "Canoe", 1),
        ("Which Philippine fruit is known as the 'poor man's apple'?", "Chico",
         "Mango", "Banana", "Pineapple", 1),
        ("What is the national dance of the Philippines?", "Tinikling",
         "Cariñosa", "Pandanggo", "Singkil", 1),
        ("Which Philippine province is known as the 'Land of the Ugs'?",
         "Quezon", "Laguna", "Cavite", "Batangas", 1),
        ("What is the traditional Filipino shirt?", "Barong Tagalog", "Kimono",
         "Saya", "Terno", 1),
        ("Which Philippine city is known as the 'Summer Capital'?", "Baguio",
         "Tagaytay", "Baguio", "Davao", 1),
        ("What is the national animal of the Philippines?", "Carabao",
         "Tamaraw", "Philippine Eagle", "Monkey", 2),
        ("Which Philippine festival features street dancing with colorful costumes?",
         "Sinulog", "Pahiyas", "Panagbenga", "Kadayawan", 1),
        ("What is the traditional Filipino breakfast?", "Tapsilog", "Adobo",
         "Sinigang", "Kare-Kare", 1),
        ("Which Philippine province is known for its pottery?", "Ilocos Sur",
         "Pampanga", "Bohol", "Albay", 1),
        ("What is the national gem of the Philippines?", "Pearl", "Diamond",
         "Ruby", "Emerald", 1),
        ("Which Philippine island is known for its whale sharks?", "Donsol",
         "Palawan", "Siargao", "Cebu", 1),
        ("What is the traditional Filipino courtship dance?", "Cariñosa",
         "Tinikling", "Pandanggo", "Kuratsa", 1),
        ("Which Philippine city is known as the 'Walled City'?", "Intramuros",
         "Vigan", "Cebu", "Davao", 1),
        ("What is the national leaf of the Philippines?", "Anahaw", "Bamboo",
         "Coconut", "Banana", 1),
        ("Which Philippine province is known for its surfing spots?",
         "Siargao", "La Union", "Baler", "Zambales", 1),
        ("What is the traditional Filipino noodle dish?", "Pancit",
         "Spaghetti", "Lasagna", "Carbonara", 1),
        ("Which Philippine hero wrote 'Noli Me Tangere'?", "Jose Rizal",
         "Andres Bonifacio", "Apolinario Mabini", "Emilio Aguinaldo", 1),
        ("What is the national costume for Filipino women?", "Baro't Saya",
         "Kimono", "Sari", "Ao Dai", 1),
        ("Which Philippine province is known for its rice terraces?", "Ifugao",
         "Benguet", "Mountain Province", "Kalinga", 1)
    ]
        ...
        if invited_by:
            c.execute(
                ...
                (user_id, username, invite_code, invited_by,
                 registration_time))
            logger.info(
                f"New user {user_id} created with referrer {invited_by}")
        else:
            c.execute(
...
def update_balance(user_id, amount):
    try:
        ...
        conn.commit()
        logger.info(
            f"Updated balance for {user_id}: +₱{amount} (New balance: {current_balance + amount})"
        )
        return True
    except Exception as e:
        logger.error(
            f"Database error in update_balance for user {user_id}: {e}")
        return False
...
def increment_invite_count(user_id):
    try:
        ...
        c.execute("SELECT invite_count FROM users WHERE user_id=?",
                  (user_id, ))
        result = c.fetchone()
        ...
        conn.commit()
        logger.info(
            f"Incremented invite count for {user_id} (New count: {current_count + 1})"
        )
        return True
    except Exception as e:
        logger.error(
            f"Database error in increment_invite_count for user {user_id}: {e}"
        )
        return False
...
def set_verified(user_id):
    try:
        ...
        if referrer and referrer[0]:
            ...
            logger.info(
                f"Rewarded referrer {referrer[0]} for verified user {user_id}")
...
def set_channel_joined(user_id):
    try:
        c.execute("UPDATE users SET channel_joined=1 WHERE user_id=?",
                  (user_id, ))
        conn.commit()
        return True
    except Exception as e:
...
def set_withdrawal_pending(user_id, status):
    try:
        c.execute("UPDATE users SET withdrawal_pending=? WHERE user_id=?",
                  (status, user_id))
        conn.commit()
        return True
    except Exception as e:
    ...
    try:
        ...
        if invites >= BONUS_QUESTIONS_THRESHOLD:
            ...
            if bonus_questions != current_bonus:
                ...
                # Award cash bonus for reaching milestone
                if bonus_questions > current_bonus:
                    update_balance(user_id, BONUS_CASH_REWARD)
                    logger.info(
                        f"Awarded ₱{BONUS_CASH_REWARD} bonus to user {user_id} for reaching {BONUS_QUESTIONS_THRESHOLD} invites"
                    )

                conn.commit()
                logger.info(
                    f"Updated bonus questions for user {user_id}: {bonus_questions}"
                )
            return True
    ...
    try:
        ...
        if not user_data:
            ...
            if context.args and context.args[0].startswith('ref'):
                try:
                    ...
                    c.execute("SELECT user_id FROM users WHERE invite_code=?",
                              (invite_code, ))
                    referrer = c.fetchone()
                    ...
                    if referrer:
                        ...
                        logger.info(
                            f"New user {user_id} came from referral by {invited_by}"
                        )
                    else:
            ...
            logger.info(
                f"Created new user {user_id} with invite code {invite_code}")
            ...
            context.bot.send_message(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [[
                        InlineKeyboardButton("📝 Register Here",
                                             url=ARENA_LIVE_LINK)
                    ],
                     [
                         InlineKeyboardButton(
                             "👥 Join Here",
                             url="https://t.me/+Z3fNuDvJO5BjNGZl")
                     ]]))
            return -1
        ...
        if not user_data[6] and can_verify(user_id):
            ...
            context.bot.send_message(
                ...
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("✅ I've Completed Registration",
            ...
            context.bot.send_message(
                ...
                reply_markup=InlineKeyboardMarkup(
                    [[
                     ...
                     [
                         InlineKeyboardButton("✅ I've Joined the Group",
                                              callback_data="group_joined")
                     ]]))
            return -1
        ...
        if user_data[6] and not user_data[10]:
            context.bot.send_message(
                ...
                parse_mode="Markdown",
                reply_markup=InlineKeyboardMarkup([[
                    InlineKeyboardButton("✅ I've Joined the Channel",
...
def start_quiz_game(update: Update, context: CallbackContext) -> int:
    try:
        ...
        if available <= 0:
            if update.message:
                update.message.reply_text(
                    ...
                    f"💡 *Invite 5 friends to get 5 bonus questions + ₱555!*\n"
        ...
        if not question_data:
            if update.message:
                update.message.reply_text(
                    "⚠️ Failed to load quiz questions. Please try again later."
                )
            return -1
...
def handle_quiz_answer(update: Update, context: CallbackContext) -> int:
    try:
        ...
        if not question_data:
            query.edit_message_text(
                "⚠️ Question data missing. Please start a new quiz.")
            return -1
...
def show_main_menu(update: Update, context: CallbackContext):
    try:
        ...
        menu_options = [["🎮 Play Captcha Game", "❓ Play Quiz Game"],
        ...
        context.bot.send_message(
            ...
            "Select an option:",
            parse_mode="Markdown",
            reply_markup=ReplyKeyboardMarkup(menu_options,
...
def verify_withdrawal_callback(update: Update,
...
def send_promotional_message():
    """Send promotional message to all verified users every 30 minutes"""
    try:
        ...
        # Create a generic invite link for the promotional message
        bot_username = "InvitorCashPhBot"  # Replace with your actual bot username
        generic_invite_link = f"https://t.me/{bot_username}"

        promotional_text = (
            "🌟 *HELP US GROW OUR COMMUNITY!* 🌟\n\n"
            "Hey there! We need your help to expand our amazing community! 🚀\n\n"
            f"📢 **Share our bot:** {generic_invite_link}\n"
            "📢 **Or share this group:** https://t.me/+Z3fNuDvJO5BjNGZl\n\n"
            "🎁 **SPECIAL REWARD OFFER!** 🎁\n"
            "Complete this mission and get **₱555 PHP** as a reward! 💰\n\n"
            "📋 **How to get your ₱555 reward:**\n"
            "1️⃣ Share our bot/group with friends\n"
            "2️⃣ Help 10 people join our community\n"
            "3️⃣ We'll automatically notify you when you reach 10 invites\n"
            "4️⃣ Claim your ₱555 reward instantly!\n\n"
            "💪 Together we can build the best earning community in the Philippines!\n\n"
            "Thank you for being part of INVITOR CASH PH! 🇵🇭")
        ...
        for user_id, invite_code in users:
            try:
                # Personalize with their invite link
                personal_invite_link = f"https://t.me/{bot_username}?start={invite_code}"
                personalized_text = promotional_text.replace(
                    generic_invite_link, personal_invite_link)
                ...
                updater.bot.send_message(
                    ...
                    reply_markup=InlineKeyboardMarkup(
                        [[
                            InlineKeyboardButton("🔗 My Invite Link",
                                                 url=personal_invite_link)
                        ],
                         [
                             InlineKeyboardButton(
                                 "👥 Join Main Group",
                ...
                failed_sends += 1
                logger.error(
                    f"Failed to send promotional message to user {user_id}: {e}"
                )
                continue

        logger.info(
            f"Promotional message sent: {successful_sends} successful, {failed_sends} failed"
        )
...
def main() -> None:
    ...
    scheduler.start()
    logger.info(
        "Promotional message scheduler started - messages will be sent every 30 minutes"
    )
    ...
    dispatcher.add_handler(
        CallbackQueryHandler(join_channel_callback, pattern='^join_channel$'))
    dispatcher.add_handler(
        CallbackQueryHandler(group_join_callback, pattern='^group_joined$'))
    dispatcher.add_handler(
        CallbackQueryHandler(verify_withdrawal_callback,
                             pattern='^verify_withdrawal$'))
    dispatcher.add_handler(
        CallbackQueryHandler(handle_quiz_answer, pattern='^quiz_'))
    dispatcher.add_handler(
        CallbackQueryHandler(next_question_callback,
                             pattern='^next_question$'))

    # Menu handlers
    dispatcher.add_handler(
        MessageHandler(Filters.regex('^🎮 Play Captcha Game$'),
                       start_captcha_game))
    dispatcher.add_handler(
        MessageHandler(Filters.regex('^❓ Play Quiz Game$'), start_quiz_game))
    dispatcher.add_handler(
        MessageHandler(Filters.regex('^👥 Invite & Earn$'), invite_friends))
    dispatcher.add_handler(
        MessageHandler(Filters.regex('^📅 Daily Sign-in$'), daily_signin))
    dispatcher.add_handler(
